...
function TarjetaPersonaje({ nombre, uid }) {
  const urlImg = `https://raw.githubusercontent.com/tbone849/star-wars-guide/refs/heads/master/build/assets/img/characters/${uid}.jpg`;
  ...
  const esFavorito = (store.favoritos || []).some(fav => fav.uid === uid && fav.tipo === "personaje");
  ...
  function guardarFavorito(e) {
    ...
    dispatch({
      ...
      payload: { tipo: "personaje", uid, nombre }
    });
  }
      ...
      <div className="card-body">
        ...
        {info && (
          <ul className="list-unstyled mb-2">
            <li><b>Género:</b> {info.gender}</li>
            <li><b>Altura:</b> {info.height} cm</li>
            <li><b>Color de pelo:</b> {info.hair_color}</li>
          </ul>
        )}
        <div className="d-flex gap-2">
          ...
          <Link to={`/detalle/personaje/${uid}`} className="btn btn-sm btn-info text-light">Saber más</Link>
        </div>
      </div>
    </div>
  );
